...
// to file 'name'.
func WriteBadData(bw *writer.BatchWriter, conv *internal.Conv, banner, driver, name string, out *os.File) {
	...
	if conv.Audit.StreamingStats.Streaming {
		badDataStreaming = getBadStreamingDataCount(conv, driver, out)
	}
	...
	if badDataStreaming > 0 {
		err = writeBadStreamingData(conv, driver, out, f)
		if err != nil {
			fmt.Fprintf(out, "Can't write out bad data file: %v\n", err)
			return
		}
...
func getBadStreamingDataCount(conv *internal.Conv, driver string, out *os.File) int64 {
...
func writeBadStreamingData(conv *internal.Conv, driver string, out, f *os.File) error {

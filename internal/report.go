...
func writeStreamingMigrationReport(driverName string, conv *Conv, w *bufio.Writer) {
	...
	w.WriteString(fmt.Sprintf("Processed %d out of %d %s Streams successfully.\n", len(stats.Records), len(orderTableNames), driverName))

	totalReadRecords := sumNestedMapValues(stats.Records)
	w.WriteString(fmt.Sprintf("\nCount of records read from %s Streams: %s\n", driverName, strconv.FormatInt(totalReadRecords, 10)))
	...
	w.WriteString(fmt.Sprintf("\nTablewise summary of processing of %s Streams (Written records / Total records)\nbroken down by record type.\n\n", driverName))

	var columns []string
	switch driverName {
	case "DynamoDB":
		columns = []string{"Table", "INSERT", "REMOVE", "MODIFY"}
	default:
	}
	...
	for _, colName := range columns {
		...
		if colName == "Table" {
			for srcTable := range stats.Records {
				if len(srcTable) > colWidth {
					colWidth = len(srcTable)
				}
			}
		} else {
			for srcTable, records := range stats.Records {
				...
				if len(progress) > colWidth {
					colWidth = len(progress)
				}
			}
		}
		colWidth += 4
	...
	for i, colName := range columns {
		spaces := colWidths[i] - len(colName)
		leftSpace := spaces / 2
	...
	for srcTable, records := range stats.Records {
		for i, colName := range columns {
			...
			spaces := colWidths[i] - len(progress)
			leftSpace := spaces / 2
			rightSpace := (spaces - leftSpace)
